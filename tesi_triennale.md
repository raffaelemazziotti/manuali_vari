# Linee guida per la tesi di laurea triennale in Psicologia

## 1. Scelta della sessione di laurea

Lo studente deve innanzitutto scegliere la sessione entro cui intende laurearsi.

Le date e le relative scadenze possono essere consultate nel calendario didattico del corso di laurea:

https://www.psicologia.unifi.it/vp-367-orario-e-calendario-didattico.html

È importante verificare con attenzione:

- la deadline per la domanda di laurea;
- le scadenze amministrative;
- le tempistiche per la consegna della tesi.

---

## 2. Contatto con il docente

Dopo aver individuato la sessione di laurea desiderata, lo studente deve contattare il docente per discutere la possibilità di svolgere la tesi.

Durante il primo incontro verranno discussi:

- l’argomento della tesi;
- la tipologia di elaborato;
- la fattibilità del progetto;
- le tempistiche di lavoro.

---

## 3. Scelta dell’argomento

Si consiglia di scegliere una tematica attuale e scientificamente rilevante.

Per individuare argomenti di interesse è possibile consultare:

- neuroTOC: https://raffaelemazziotti.github.io/neuroTOC/
- PubMed: https://pubmed.ncbi.nlm.nih.gov/

Il sito neuroTOC non è obbligatorio, ma rappresenta uno strumento utile per monitorare articoli recenti nell’ambito delle neuroscienze e della psicobiologia.

### Indicazioni bibliografiche iniziali

All’inizio del lavoro è fortemente consigliato:

- concentrarsi principalmente su review;
- cercare review recenti;
- esplorare tematiche realmente vicine ai propri interessi.

Le review permettono di acquisire rapidamente una visione generale del campo e di comprendere meglio lo stato dell’arte della ricerca.

### Fonti da evitare

Si raccomanda di evitare l’utilizzo di articoli provenienti da bioRxiv come principale riferimento bibliografico, poiché la piattaforma contiene prevalentemente preprint non ancora sottoposti a revisione paritaria.

---

## 4. Tipologie di tesi

La tesi può essere:

- **compilativa**
- **sperimentale**

### Tesi compilativa

La tesi compilativa deve essere concordata con il docente.

Vengono generalmente accettate tesi di psicobiologia focalizzate su:

- meccanismi di psicobiologia comparata;
- modelli animali e uomo;
- approcci translazionali;
- etologia e neuroetologia;
- fisiologia;
- elettrofisiologia;
- tecniche di imaging.

### Tesi sperimentale

La tesi sperimentale viene concordata con il docente sulla base delle disponibilità del laboratorio e dei progetti in corso.

Sono comunque benvenute proposte da parte degli studenti.

---

## 5. Strumenti di lavoro

### Scrittura della tesi

La tesi verrà redatta tramite Google Docs.

Successivamente verranno fornite istruzioni dettagliate riguardo:

- struttura del documento;
- modalità di condivisione;
- revisione del testo;
- formattazione

Prima di cominciare a scrivere la tesi, contattare il relatore.

### Gestione della bibliografia

Per la gestione delle citazioni bibliografiche verrà utilizzato Paperpile, integrato con Google Docs.

Per l’installazione e la configurazione di Paperpile è necessario rivolgersi direttamente al docente.

Paperpile consente di:

- organizzare gli articoli;
- inserire citazioni automaticamente;
- generare la bibliografia finale.

---

## 6. Regole generali di formattazione

Lo studente deve consultare anche le linee guida ufficiali del corso di laurea, che includono indicazioni sul frontespizio e sulla struttura generale della tesi:

https://www.psicologia.unifi.it/upload/sub/LINEE%20GUIDA%20TESI%20LM-51_10_04_26-docx.pdf

### Struttura del documento

La tesi deve essere formattata correttamente fin dall’inizio. Ogni elemento del testo deve essere classificato con il tag appropriato in Google Docs:

- titolo del documento;
- titoli principali come Heading 1;
- sottosezioni come Heading 2;
- eventuali sottoparagrafi come Heading 3;
- testo principale come paragrafo.

Questo è necessario per generare automaticamente l’indice della tesi.

### Indice e bibliografia

L’indice deve essere generato automaticamente usando gli strumenti di Google Docs.

La sezione bibliografia deve essere inserita alla fine della tesi e classificata come Heading 1.

### Formattazione del testo

Il testo deve essere impostato come paragrafo normale e non giustificato a sinistra.

### Immagini e dati

Una tesi senza dati o immagini esemplificative non può essere consegnata.

Anche le tesi compilative devono includere:

- immagini;
- schemi;
- grafici;
- figure esplicative;
- eventuali dati presenti in letteratura.

### Lunghezza massima

Il limite massimo della tesi è di 25 pagine, esclusa la bibliografia.

Nel conteggio delle pagine devono essere incluse anche le immagini e le figure.

---

## 7. Scadenze per la revisione

La consegna della tesi al docente per la correzione finale deve avvenire non oltre due settimane prima della deadline ufficiale per la domanda di laurea.

Consegne effettuate oltre questa scadenza potrebbero non garantire tempi sufficienti per la revisione completa dell’elaborato.
